...
def answer(fname: str, messages: list[dict], query: str) -> tuple[str, list[dict]]:
    """Generates response from file, message history, and query
    ...
    code_prompt = {"role": "system", "content":  "Answer questions about the following code:\n" + code}
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[system_prompt, code_prompt, *messages]
        )
    except:
...
def multi_answer(file_set: set[str], messages: list[dict], query: str) -> tuple[str, list[dict]]:
